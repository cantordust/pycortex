...
def calibrate():
    ...
    for species in Species.Populations.values():
        ...
        if (Net.Champion is None or
            Net.Ecosystem[species.champion].fitness.absolute > top_fitness):

            Net.Champion = species.champion
            top_fitness = Net.Ecosystem[species.champion].fitness.absolute
...
def cull():
    ...
    while len(Net.Ecosystem) > Net.Max.Count:
        ...
        for net_id in Species.Populations[species_id].nets:
            ...
            if net.age > 0:
                net_wheel.add(net_id, Net.Ecosystem[net_id].fitness.relative)
...
#            CurrentEpoch)
        ...
        Logger.add_scalar('Highest fitness', Net.Ecosystem[Net.Champion].fitness.absolute, CurrentEpoch)
        Logger.add_scalar('Networks', len(Net.Ecosystem), CurrentEpoch)
        Logger.add_scalar('Species', len(Species.Populations), CurrentEpoch)

        if Net.Champion is not None:
            save(Net.Champion, 'champion')

        _stats['Parameters'].update(Net.Ecosystem[Net.Champion].get_parameter_count())
        _stats['Accuracy'].update(Net.Ecosystem[Net.Champion].fitness.absolute)
...
def run():
    ...
    for run in range(Runs):
        ...
        for epoch in range(Epochs):
            ...
            with context.Pool(processes = MaxThreads) as pool:
